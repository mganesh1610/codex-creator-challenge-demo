...
function renderRuntimeBanner(status = {}) {
  ...
  const latestImport = status.latest_import || null;
  const latestImportText = latestImport
    ? `${latestImport.status} | ${latestImport.file_name} | ${formatTimestamp(latestImport.finished_at || latestImport.started_at)}`
    : status.import_error
      ? `Unavailable | ${status.import_error}`
      : "No imports recorded yet.";

  const items = [
    {
      label: "Data",
      value: status.db_ready ? "Connected" : "Not configured",
      detail: status.db_ready ? "Demo data source is available." : "Synthetic demo data is loaded in-memory for this public build.",
      tone: status.db_ready ? "ready" : "warn",
    },
    {
      label: "Query AI",
      value: status.query_ai_enabled ? `${providerLabel(status.ai_provider)} connected` : "Disabled",
      detail: status.query_ai_enabled
        ? "Natural-language SQL generation is enabled for explicit query requests."
        : "The query panel will fall back to deterministic projection SQL.",
      tone: status.query_ai_enabled ? "ready" : "warn",
    },
    {
      label: "Preview Gate",
      value: "Review before import",
      detail: "Preview flags site, visit, date, and disposition issues before anything is imported.",
      tone: "ready",
    },
    {
      label: "Last Import",
      value: latestImport ? latestImport.status : "None",
      detail: latestImportText,
      tone: latestImport && latestImport.status === "SUCCEEDED" ? "ready" : "warn",
    },
  ];

  runtimeBanner.innerHTML = items
    .map(
      (item) => `
        <article class="banner-item ${item.tone}">
